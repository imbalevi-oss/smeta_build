...
/*   это вариант без доп фильтров на мусор и на 0.9
...
export async function copyFilteredCodes() {
    ...
    try {
        ...
        // ========== ФУНКЦИЯ ПРОВЕРКИ, ЯВЛЯЕТСЯ ЛИ ОПИСАНИЕ ЗАГОЛОВКОМ СБОРНИКА/ГЛАВЫ ==========
        function isHeadingDescription(description) {
            if (!description || description === '—') return false;
            
            const lowerDesc = description.toLowerCase();
            
            // Паттерны для заголовков сборников и глав
            const headingPatterns = [
                /^глава\s+\d+/i,           // "Глава 1", "глава 2" и т.д.
                /^лава\s+\d+/i,            // "лава 1" (с опечаткой)
                /^сборник\s+\d+/i,         // "Сборник 10", "Сборник 1"
                /^раздел\s+\d+/i,          // "Раздел 1"
                /^часть\s+\d+/i,           // "Часть 1"
                /^отдел\s+\d+/i,           // "Отдел 1"
                /^таблица\s+\d+/i,         // "Таблица 1"
                /^приложение\s+\d+/i,      // "Приложение 1"
                /^\.\s*глава\s+\d+/i,      // ". Глава 1"
                /^\.\s*лава\s+\d+/i,       // ". лава 1"
                /^\.\s*сборник\s+\d+/i     // ". Сборник 10"
            ];
            
            // Проверяем начало строки (первые 60 символов обычно достаточно)
            const startOfDesc = lowerDesc.substring(0, 60);
            
            for (const pattern of headingPatterns) {
                if (pattern.test(startOfDesc)) {
                    return true;
                }
            }
            
            // Дополнительная проверка: если строка короткая и содержит ключевые слова
            const shortHeadingKeywords = ['глава', 'лава', 'сборник', 'раздел', 'часть', 'отдел'];
            const words = lowerDesc.split(/\s+/).filter(w => w.length > 0);
            
            if (words.length <= 5) { // Короткая строка
                for (const keyword of shortHeadingKeywords) {
                    if (lowerDesc.includes(keyword) && /\d/.test(lowerDesc)) { // есть цифра
                        return true;
                    }
                }
            }
        ...
        // ========== ПРОВЕРКА НАЛИЧИЯ КОДА 1.50 (ТОЛЬКО ТОЧНЫЕ СОВПАДЕНИЯ) ==========
        const code150ExactList = [
            '1.50-3203-33-1/1',
            '1.50-3203-33-2/1',
            '1.50-3203-32-1/1',
            '1.50-3203-32-2/1',
        ...
        const hasCode150 = allPositions.some(item => {
            const code = item.extractedCode || item.code || '';
            return code150ExactList.includes(code);
        });

        // ========== ОТЛАДКА ==========
        console.log('===== ДЕБАГ КОПИРОВАНИЯ =====');
        console.log('Есть точный код 1.50 в смете:', hasCode150);
        console.log('Всего позиций в allPositions:', allPositions.length);

        // ========== МАССИВЫ ДЛЯ ПРОВЕРКИ КОДОВ 1.13 ==========
        const code113Prefixes = [
            // 1.13-3103 (21 код)
            ...
            // Остальные
            '1.13-3503-1', '1.13-3503-3', '1.13-3603-2', '1.13-3603-22',
            '1.13-3501-1', '1.13-3501-2'
        ];

        const code113ExactList = [
            '1.13-3101-8-1/1',
            '1.13-3101-7-8/1',
            '1.13-3101-7-7/1',
            '1.13-3101-7-6/1',
            ...
            '1.13-3503-1-1/2',
            '1.13-3503-1-1/1',
            '1.13-3103-1-1/1', '1.13-3103-1-2/1', '1.13-3103-1-3/1', '1.13-3103-1-4/1',
            '1.13-3103-1-5/1', '1.13-3103-1-6/1', '1.13-3103-1-7/1', '1.13-3103-1-8/1',
            '1.13-3103-1-9/1', '1.13-3103-1-10/1', '1.13-3103-1-11/1', '1.13-3103-1-12/1',
            '1.13-3103-2-1/1', '1.13-3103-2-2/1', '1.13-3103-2-3/1', '1.13-3103-2-4/1',
            '1.13-3103-2-5/1', '1.13-3103-2-6/1', '1.13-3103-2-7/1', '1.13-3103-2-8/1',
            '1.13-3103-2-9/1', '1.13-3103-2-10/1', '1.13-3103-2-11/1', '1.13-3103-2-12/1',
            '1.13-3103-3-1/1', '1.13-3103-3-2/1', '1.13-3103-3-3/1',
            '1.13-3103-4-1/1', '1.13-3103-4-2/1',
            '1.13-3103-5-1/1', '1.13-3103-5-2/1',
            '1.13-3103-6-1/1', '1.13-3103-6-2/1', '1.13-3103-6-3/1', '1.13-3103-6-4/1',
            '1.13-3103-6-5/1', '1.13-3103-6-6/1', '1.13-3103-6-7/1',
            '1.13-3103-7-1/1',
            '1.13-3103-8-1/1', '1.13-3103-8-2/1',
            '1.13-3103-9-1/1', '1.13-3103-9-2/1', '1.13-3103-9-3/1', '1.13-3103-9-4/1',
            '1.13-3103-10-1/1', '1.13-3103-10-2/1', '1.13-3103-10-3/1', '1.13-3103-10-4/1',
            '1.13-3103-11-1/1', '1.13-3103-11-2/1', '1.13-3103-11-3/1', '1.13-3103-11-4/1',
            '1.13-3103-11-5/1', '1.13-3103-11-6/1',
            '1.13-3103-12-1/1', '1.13-3103-12-2/1', '1.13-3103-12-3/1', '1.13-3103-12-4/1',
            '1.13-3103-12-5/1', '1.13-3103-12-6/1',
            '1.13-3103-13-1/1', '1.13-3103-13-2/1', '1.13-3103-13-3/1', '1.13-3103-13-4/1',
            '1.13-3103-13-5/1', '1.13-3103-13-6/1',
            '1.13-3103-14-1/1', '1.13-3103-14-2/1', '1.13-3103-14-3/1', '1.13-3103-14-4/1',
            '1.13-3103-16-1/1', '1.13-3103-16-2/1',
            '1.13-3103-17-1/1', '1.13-3103-17-2/1', '1.13-3103-17-3/1', '1.13-3103-17-4/1',
            '1.13-3103-17-5/1', '1.13-3103-17-6/1', '1.13-3103-17-7/1', '1.13-3103-17-8/1',
            '1.13-3103-17-9/1', '1.13-3103-17-10/1', '1.13-3103-17-11/1', '1.13-3103-17-12/1',
            '1.13-3103-19-1/1', '1.13-3103-19-2/1', '1.13-3103-19-3/1', '1.13-3103-19-4/1',
            '1.13-3103-19-5/1', '1.13-3103-19-6/1', '1.13-3103-19-7/1', '1.13-3103-19-8/1',
            '1.13-3103-19-9/1', '1.13-3103-19-10/1', '1.13-3103-19-11/1', '1.13-3103-19-12/1',
            '1.13-3103-20-1/1', '1.13-3103-20-2/1',
            '1.13-3103-21-1/1', '1.13-3103-21-2/1', '1.13-3103-21-3/1', '1.13-3103-21-4/1',
            '1.13-3103-21-5/1', '1.13-3103-21-6/1'
        ];
        ...
        function shouldIncludeCode(item) {
            ...
            // ========== 3. КОДЫ 1.13 (только из массивов) ==========
            const isCode113 = code113ExactList.includes(code) || 
                              code113Prefixes.some(prefix => code.startsWith(prefix));
            
            if (isCode113) {
                // Если есть код 1.50 в смете - выводим 1.13, иначе НЕ выводим
                return hasCode150;
            }
        ...
        console.log(`📊 Всего отфильтровано позиций: ${codesToInclude.length}`);
...
`;
            for (let i = 0; i < notAllowedCodes.length; i++) {
                ...
                if (fullDescription && fullDescription !== '—') {
                    ...
                    // Пропускаем, если это заголовок сборника или главы
                    if (cleanDesc && !isHeadingDescription(cleanDesc)) {
                        line += `\n    ${cleanDesc}`;
                    }
                }
                text += line + '\n';
...
`;
            for (let i = 0; i < warningCodes.length; i++) {
                ...
                // Пропускаем, если это заголовок сборника или главы
                if (cleanDesc && cleanDesc !== '—' && cleanDesc !== '' && !isHeadingDescription(cleanDesc)) {
                    line += `\n    ${cleanDesc}`;
                }
                text += line + '\n';
            }
